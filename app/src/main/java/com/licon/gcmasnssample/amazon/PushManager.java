...
import com.licon.gcmasnssample.firebase.FCMTokenHelper;
import com.licon.gcmasnssample.utils.ThreadUtils;
...
/**
 * The Push Manager registers the FCM device token in Amazon SNS. The result of this
...
public class PushManager implements FCMTokenHelper.FCMTokenUpdateObserver {
    ...
    private final FCMTokenHelper mFCMTokenHelper;
     ...
     * @param FCMTokenHelper the manager of the gcm token.
    ...
    public PushManager(final Context context,
                       final FCMTokenHelper FCMTokenHelper,
        ...
        this.mFCMTokenHelper = FCMTokenHelper;
    ...
    @Override
    public void onFCMTokenUpdate(final String fcmToken, final boolean didTokenChange) {
        if (didTokenChange || !isRegistered()) {
            try {
                Log.d(LOG_TAG, "FCM Token changed or SNS endpoint not registered.");
                try {
                    createPlatformArn();
    ...
    @Override
    public void onFCMTokenUpdateFailed(final Exception ex) {
        Log.e(LOG_TAG, "Push Notifications - FAILED : FCM registration failed : " + ex, ex);
        pushEnabled = false;
        informStateListener();
    }
    ...
    public void registerDevice() {
        // Updates the FCM token, which triggers {@link #onGCMTokenUpdate(String,boolean)} to
        // create the platform
        // arn set push enabled, and re-subscribe to any previously subscribed topics.
        mFCMTokenHelper.updateFCMToken();
    }
    ...
    private void createPlatformArn() {
        ...
        request.setToken(mFCMTokenHelper.getFCMToken());
